...
// DeathFade fades a dying minion's opacity to 0 and calls onDone when
// the minion should be removed from the board.
...
func NewDeathFade(duration float64, opacity *float64, onDone func()) *DeathFade {
	return &DeathFade{
		duration: duration,
		opacity:  opacity,
		onDone:   onDone,
...
func NewDeathTint(opacity *float64, deathReason game.DeathReason) *DeathTint {
...
func (e *DeathTint) Update(float64) bool {
	return *e.opacity <= 0
}
...
func (e *DeathTint) DrawFront(screen *ebiten.Image, res ui.Resolution, rect ui.Rect) {
	...
	a := uint8(float64(80) * (1.0 - *e.opacity))
	clr := e.tintColor
	clr.A = a
