...
type Sidebar struct {
	...
	snap      []ui.PlayerEntry // frozen player list
}
...
func (s *Sidebar) players() []ui.PlayerEntry {
	return s.snap
}

// Update processes tier-fade animations, opponent updates, and hover detection.
func (s *Sidebar) Update(res ui.Resolution, rect ui.Rect, players []ui.PlayerEntry, updates []api.OpponentUpdate, dt float64) {
	if players != nil {
		s.snap = players
	}
	...
	s.updateHover(res, rect)
}

func (s *Sidebar) updateHover(res ui.Resolution, rect ui.Rect) {
	s.hover = -1
...
func (s *Sidebar) Draw(screen *ebiten.Image, res ui.Resolution, rect ui.Rect, player, opponent game.PlayerID) {
	s.drawList(screen, res, rect, player, opponent)
	s.drawTooltip(screen, res, rect)
}

func (s *Sidebar) drawList(screen *ebiten.Image, res ui.Resolution, rect ui.Rect, player, opponent game.PlayerID) {
	sr := rect.Screen(res)
	...
	for i, e := range players {
		...
		ui.DrawText(screen, res, s.font, fmt.Sprintf("%d  %d HP", e.ID, e.HP), row.X+padX, row.Y+rowH*0.2, nameClr)
		...
		ui.DrawText(screen, res, s.font, line2, row.X+padX, row.Y+rowH*0.55, color.RGBA{160, 160, 180, 255})
		...
		if f, ok := s.tierFades[e.ID]; ok {
			...
			ui.DrawText(screen, res, s.font, tierStr, row.X+row.W*0.70, row.Y+rowH*0.35, color.RGBA{255, 215, 0, alpha})
		}
...
func (s *Sidebar) drawTooltip(screen *ebiten.Image, res ui.Resolution, rect ui.Rect) {
	if s.hover < 0 {
		return
	}
	players := s.players()
	...
	// Header: player name.
	ui.DrawText(screen, res, s.font, fmt.Sprintf("%d", e.ID), tip.X+padX, tip.Y+tip.H*0.08, color.RGBA{220, 220, 220, 255})

	// Tribe info.
	switch e.TopTribe {
	case game.TribeNeutral:
	case game.TribeMixed:
		ui.DrawText(screen, res, s.font, "Mixed", tip.X+padX, tip.Y+tip.H*0.22, color.RGBA{180, 180, 200, 255})
	default:
		tribeStr := fmt.Sprintf("%s x%d", e.TopTribe, e.TopTribeCount)
		ui.DrawText(screen, res, s.font, tribeStr, tip.X+padX, tip.Y+tip.H*0.22, color.RGBA{180, 180, 200, 255})
	}
	...
	for _, cr := range e.CombatResults {
		...
		ui.DrawText(screen, res, s.font, label, tip.X+padX, y, clr)
		y += lineH
	}

	if len(e.CombatResults) == 0 {
		ui.DrawText(screen, res, s.font, "No fights yet", tip.X+padX, tip.Y+tip.H*0.40, color.RGBA{100, 100, 100, 255})
	}
}
