...
// UpdateEffects advances persistent keyword effect animations.
func (r *CardRenderer) UpdateEffects(elapsed float64) {
	if r.windfury != nil {
		r.windfury.Update(elapsed)
	}
}
...
func (r *CardRenderer) drawKeywordEffects(
	...
	if keywords.Has(game.KeywordWindfury) {
		if r.windfury == nil {
			r.windfury = effect.NewWindfury()
		}
		r.windfury.Alpha = alpha
		r.windfury.DrawFront(screen, r.Res, rect)
	}
...
func (r *CardRenderer) drawEllipseBase(
	...
	if keywords.Has(game.KeywordWindfury) {
		...
		r.windfury.Alpha = alpha
		r.windfury.DrawBehind(screen, r.Res, rect)
	}
